...
func TestListChargePointSimple(t *testing.T) {
	...
	res, chargePoint1 := createChargePoint(t, `{"id": "cp1", "model": "model1", "vendor": "vendor1"}`)
	assert.Equal(t, 201, res.StatusCode)

	// Create the chargePoint
	res, chargePoint2 := createChargePoint(t, `{"id": "cp1", "model": "model2", "vendor": "vendor2"}`)
	assert.Equal(t, 201, res.StatusCode)
	...
	// Delete it
	res = deleteChargePoint(t, chargePoint1.ExternalId)
	assert.Equal(t, 200, res.StatusCode)
	res = deleteChargePoint(t, chargePoint2.ExternalId)
	assert.Equal(t, 200, res.StatusCode)
}
...
func TestCreateChargePointSimple(t *testing.T) {
	// Create the chargePoint
	res, chargePoint := createChargePoint(t, `{"id": "cp3", "model": "model1", "vendor": "vendor1"}`)
	assert.Equal(t, 201, res.StatusCode)

	// Delete it
	res = deleteChargePoint(t, chargePoint.ExternalId)
	assert.Equal(t, 200, res.StatusCode)
}
...
func TestCreateChargePointFull(t *testing.T) {
	...
	r, err := http.NewRequest("GET", chargePointsBaseUrl+"/"+chargePoint1.ExternalId, nil)
	assert.Nil(t, err)
	res, err = http.DefaultClient.Do(r)
	assert.Nil(t, err)
	assert.Equal(t, 200, res.StatusCode)
	...
	assert.Nil(t, err)
	assert.Equal(t, chargePoint1.ExternalId, chargePoint2.ExternalId)
...
func TestCreateChargePointMissingModel(t *testing.T) {
	// Create the chargePoint
	res, _ := createChargePoint(t, `{"id": "cp1", "vendor": "vendor1"}`)
...
func TestCreateChargePointMissingVendor(t *testing.T) {
	// Create the chargePoint
	res, _ := createChargePoint(t, `{"id": "cp1", "model": "model1"}`)
	assert.Equal(t, 400, res.StatusCode)
}

// Test creation of ChargePoint, missing Id
func TestCreateChargePointMissingId(t *testing.T) {
	// Create the chargePoint
	res, _ := createChargePoint(t, `{"model": "model1", "vendor": "vendor"}`)
	assert.Equal(t, 400, res.StatusCode)
}
...
func TestDeleteChargePointSimple(t *testing.T) {
	// Create the chargePoint
	res, chargePoint := createChargePoint(t, `{"id": "cpdelete", "model": "model1", "vendor": "vendor1"}`)
	assert.Equal(t, 201, res.StatusCode)
...
func TestUpdateChargePoint(t *testing.T) {
	...
	r, err = http.NewRequest("PUT", chargePointsBaseUrl+"/"+chargePoint1.ExternalId, reader)
	assert.Nil(t, err)
	res, err = http.DefaultClient.Do(r)
	assert.Nil(t, err)
	assert.Equal(t, 200, res.StatusCode)
	...
	assert.Nil(t, err)
	assert.Equal(t, "cp1", chargePoint3.ExternalId)
	...
	// Delete it again
	r, err = http.NewRequest("DELETE", chargePointsBaseUrl+"/"+chargePoint1.ExternalId, nil)
	assert.Nil(t, err)
	res, err = http.DefaultClient.Do(r)
	assert.Nil(t, err)
	assert.Equal(t, 200, res.StatusCode)
